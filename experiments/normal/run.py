...
# testing data
X_test, Y_test, Yc_test = generate_data(N=500, seed=1, equally_spread=True)
...
for exp, prior_invstrength in prior_invstrengths.items():
    ...
    # prior data
    X_prior, Y_prior, Yc_prior = generate_data(N=100, seed=2, equally_spread=True, sd_factor=prior_invstrength)
...
# PLOT RESULTS
plt.gca()
fig, axes = plt.subplots(2, len(bgfr_results), figsize=(4*len(bgfr_results), 8), squeeze=False, sharey="row")
for i, (exp, res) in enumerate(bgfr_results.items()):
    # Plot estimation error
    ax = axes[0, i]
    ax.set_title(f"Prior strength: {exp}")
    ax.axhline(y=gfr_est_error, color="blue", linestyle="--", label="GFR")
    sns.lineplot(data=res["results"], x="reg", y="est_error", ax=ax, color="red", label="BGFR")
    ax.set_xscale("log")
    ax.set_xlabel("Regularization")
    if i < len(bgfr_results) - 1:
    ...
    X_scale, Y_scale, reg = res["best"]
    df = pd.DataFrame({
        "mu": Y_train[:, 0].detach().cpu().numpy(),
        "sigma": Y_train[:, 1].detach().cpu().numpy(),
        "ratio": ratio,
    })
    ax.plot(res["Y_prior"][:, 0], res["Y_prior"][:, 1], "ko", alpha=0.25, markersize=3, label="prior")
    sns.scatterplot(data=df, x="mu", y="sigma", hue="ratio", palette="rocket_r", ax=ax, legend=True, zorder=10)
    ax.plot(Ygfr_pred[:, 0], Ygfr_pred[:, 1], "bo", label="gfr", markersize=3, alpha=0.2)
    ax.plot(Y[:, 0], Y[:, 1], "ro", label="bgfr", markersize=3, alpha=0.2)
    ax.set_title(f"X_scale: {X_scale} | Y_scale: {Y_scale} | reg: {reg}")
plt.tight_layout()
plt.savefig(DIRFIGURES + "results.pdf")
# ----------------------------------------------------------------------------------------------------------------------