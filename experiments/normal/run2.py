...
for i, (exp, res) in enumerate(bgfr_results.items()):
    ...
    ax = axes[0]
    ax.axhline(y=gfr_est_error, color="lightblue", linestyle="--", label="GFR")
    sns.lineplot(data=res["results"], x="reg", y="est_error", ax=ax, color=blue, label="BGFR")
    ax.set_xscale("log")
    ...
    ax.plot(Ygfr_pred[:, 0], Ygfr_pred[:, 1], "o", markersize=3, alpha=1., label="GFR prediction", c=lightblue)
    ...
    Y = res["best_pred"]
    ax.plot(Y[:, 0], Y[:, 1], "o", markersize=3, alpha=1., label="BGFR prediction", c=blue)
