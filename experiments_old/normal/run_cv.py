...
regs = torch.logspace(-4, 1., 100).tolist()  # 100 values from 0.0001 to 10
# Fit parameters
max_iter = 100
lr = 0.05
# ----------------------------------------------------------------------------------------------------------------------
...
def bgfr_predict(
        ...
        reg=0.001, lr=0.05, max_iter=100
):
    ...
    for i in range(max_iter):
        ...
        prev_loss = loss.item()
        prev_Y = Y.clone().detach()
    return Y.clone().detach(), rxy_train
...
for seed in range(n_seeds):
    ...
    for exp, prior_invstrength in prior_invstrengths.items():
        ...
        for X_scale, Y_scale, reg in itertools.product(X_scales, Y_scales, regs):
            ...
            Y, rxy_train = bgfr_predict(
                X_train, Y_train, Yc_train,
                X_test, Y_test, Yc_test,
                X_prior, Y_prior, Yc_prior,
                Dx_median, Dy_median,
                X_scale=X_scale, Y_scale=Y_scale,
                reg=reg, lr=lr, max_iter=max_iter
            )
            ...
            for fold in range(n_folds):
                ...
                cv_error += squared_L2_distance(Y_fold, Y_train[foldid == fold]).diag().sum().item()



            # Store results
            exp_results[(X_scale, Y_scale, reg)] = pred_error, est_error, cv_error
        ...
        exp_results.columns = ["X_scale", "Y_scale", "reg", "pred_error", "est_error", "cv_pred_error"]
        # Store best result
...
for exp, prior_invstrength in prior_invstrengths.items():
    ...
    for seed in range(n_seeds):
        ...
        res["rel_error"] = res["est_error"] / gfr_results[seed]["gfr_est_error"]
        sns.lineplot(data=res, x="reg", y="rel_error", ax=ax,
                     color=lightblue, label=("BGFR" if seed==0 else None), alpha=0.2)
        # add lowest value
        minval = res["cv_pred_error"].min()
        minreg = res.loc[res["cv_pred_error"] == minval, "reg"].values[0]
        val = res.loc[res["cv_pred_error"] == minval, "rel_error"].values[0]
        ax.plot(minreg, val, "o", color=lightblue, markersize=5, alpha=1.)
    ax.set_xscale("log")
